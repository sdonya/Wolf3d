...
/*   Updated: 2020/11/05 17:55:35 by spalmer          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "wolf.h"

void	check_side(t_man *man)
{
	if (man->flag_side != 5 && man->flag_side != 6)
	{
		if (man->hor > man->ver)
		{
...
void	build_labirint(t_all *wolf, int x, char **ar)
{
	...
	if (wolf->man.hor > wolf->man.ver)
	{
		...
		if (ar[(int)wolf->man.vercast.by][(int)wolf->man.vercast.bx] == 'G')
			wolf->man.flag_side = 6;
	}
	else
	{
		len = wolf->man.hor;
		if (ar[(int)wolf->man.horcast.ay][(int)wolf->man.horcast.ax] == 'P')
			wolf->man.flag_side = 5;
		if (ar[(int)wolf->man.horcast.ay][(int)wolf->man.horcast.ax] == 'G')
			wolf->man.flag_side = 6;
	}
	check_side(&wolf->man);
	...
	render_textures(&wolf->img, &wolf->render, &wolf->man);
}
