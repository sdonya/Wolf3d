...
/*   Updated: 2020/11/05 19:06:56 by sdonya           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (line[i] != '1' && line[i] != ' ' &&
			norm->strok == 0 && line[i] != 'P' && line[i] != 'G')
		return (0);
	else if (((line[0] != '1' || line[j - 1] != '1') && (line[0] != 'P' \
	|| line[j - 1] != 'P') && (line[0] != 'P' || line[j - 1] != '1') \
	&& (line[0] != '1' || line[j - 1] != 'P') && (line[0] != 'G' \
	|| line[j - 1] != 'G') && (line[0] != 'G' || line[j - 1] != '1') \
	&& (line[0] != '1' || line[j - 1] != 'G') \
	&& (line[0] != 'P' || line[j - 1] != 'G') \
	&& (line[0] != 'G' || line[j - 1] != 'P')) && norm->strok != 0)
		return (0);
	else if (line[i] != '1' && line[i] != ' ' \
	&& norm->strok == norm->height - 1 && line[i] != 'P' && line[i] != 'G')
		return (0);
	return (1);
}
...
int			checkline(char *line, int *flag, t_norm *norm)
{
	...
	while (line[i] != '\0')
	{
		if (!chek_ones(line, norm, i, j))
			return (0);
		if ((line[i] == '1' || line[i] == 'P' || line[i] == 'G') \
		&& line[i + 1] != '\0')
			if (line[i + 1] == '1' || line[i + 1] == 'P' || line[i + 1] == 'G')
				return (0);
		if (line[i] != ' ' && line[i] != '1' && line[i] != '0' && \
		line[i] != 'W' && line[i] != 'P' && line[i] != 'G')
			return (0);
		if (line[i] == 'W')
			(*flag)++;
		i++;
